...
# llm = GPTAPI(model_type="claude-3-sonnet", key=API_KEYS['claude'],openai_api_base="https://api.xty.app/v1/chat/completions")
...
# 初始化数组 A[0..1500]
A = [None] * 1501
A[0] = '''你需要运用searcher工具完成以下单项选择题,最终回复明确选项,如1.A .你一定需要将题目和选项都作为root_node的content,避免遗漏信息.
...
with open('assets/answers1.txt', 'w', encoding='utf-8') as f:
    for i in trange(1,11,desc="Question"):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                for agent_return in agent.stream_chat(
                    question =  A[0] + '\n' + A[i]):
                    pass  
