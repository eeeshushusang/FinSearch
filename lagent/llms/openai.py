...
class GPTAPI(BaseAPIModel):
    ...
    def generate_request_data(self,
        ...
        if model_type.lower().startswith('gpt'):
            ...
            # Remove 'top_k' if it exists in gen_params
        ...
        elif model_type.lower().startswith('claude') or model_type.lower().startswith('deepseek'):
            gen_params.pop('top_k',None)
            gen_params.pop('skip_special_tokens', None)
            gen_params.pop('session_id', None)
            data = {
            ...
            if json_mode:
                data['response_format'] = {'type': 'json_object'}
        elif model_type.lower().startswith('qwen')or self.model_type.lower().startswith('Qwen'):
            header['X-DashScope-SSE'] = 'enable'
            gen_params.pop('skip_special_tokens', None)
            gen_params.pop('session_id', None)
            if 'repetition_penalty' in gen_params:
                if gen_params['repetition_penalty'] <= 0.0:
                    gen_params['repetition_penalty'] = 0.5  # 设置默认值
            else:
                gen_params['repetition_penalty'] = 0.5  # 设置默认值
